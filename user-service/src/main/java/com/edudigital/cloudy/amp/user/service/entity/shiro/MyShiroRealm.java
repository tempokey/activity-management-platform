...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		UserDTO user = userDAO.getUserPermission(userDTO);// 实际项目中，这里可以根据实际情况做缓存，如果不做，Shiro自己也是有时间间隔机制，2分钟内不会重复执行该方法
		if (user != null) {
			// 权限信息对象info,用来存放查出的用户的所有的角色（role）及权限（permission）
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			// 用户的角色集合
			info.setRoles(user.listRoleNames());
			// 用户的角色对应的所有权限，如果只使用角色定义访问权限，下面的四行可以不要
			List<RoleDTO> roleList = user.getRoles();
			for (RoleDTO role : roleList) {
				info.addStringPermissions(role.listPermissionNames());
			}
			// 或者按下面这样添加
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken)
		...
		UserDTO userDTO = new UserDTO();
		userDTO.setAccount(token.getUsername());
		// 查出是否有此用户
		UserDTO user = userDAO.getUserPermission(userDTO);
		if (user != null) {
